...
export function VoiceInterface({
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onresult = (event: any) => {
      ...
      if (finalText.trim()) {
        // For rating: immediately abort and process (no delay)
        if (ratingPhaseRef.current === "listening" || ratingPhaseRef.current === "invalid") {
          recognition.abort(); // Use abort() instead of stop() for immediate halt
          handleRatingResponse(finalText.trim());
        } else {
          // For queries: use timeout to allow multi-word sentences
  ...
  const handleRatingResponse = useCallback(
    (text: string) => {
      ...
      // Try to match spoken numbers first
      for (const [word, num] of Object.entries(numberMap)) {
        if (lower.includes(word)) {
          console.log("Voice rating:", num, `(from "${word}")`);
          setHasRated(true);
          setRatingPhase("done");
          const thankText = `Thank you for rating us ${num} out of 5! We appreciate your feedback. Have a great day!`;
          setAiText(thankText);
          speak(thankText, () => {
            setRatingPhase("none");
          });
      ...
      // Fall back to digit matching
      const digitMatch = text.match(/\b([1-5])\b/);
      if (digitMatch) {
        const rating = parseInt(digitMatch[1], 10);
